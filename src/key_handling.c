...
{
	if (info->map[(int)player->y][(int)(player->x + player->dir_x * move_speed)] != '1')
		player->x += player->dir_x * move_speed;
	if (info->map[(int)(player->y + player->dir_y * move_speed)][(int)player->x] != '1')
		player->y += player->dir_y * move_speed;
}

void		move_player_left_right(t_info *info, t_player *player, double move_speed)
{
	if (info->map[(int)player->y][(int)(player->x + player->dir_y * move_speed)] != '1')
		player->x += player->dir_y * move_speed;
	if (info->map[(int)(player->y + -player->dir_x * move_speed)][(int)player->x] != '1')
...
void		rotate_player(t_player *player, double rotate_speed)
{
	...
	player->dir_y = old_vec_dir_x * sin(rotate_speed) + player->dir_y * cos(rotate_speed);
